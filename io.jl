...
function OCO_read_inputs_from_l1b_and_met(
    ...
    met_dicts = RE.OCO_pull_atmosphere_from_met(met_h5, sounding_id=sounding_id)
    # Get the scene locations as location objects
    loc_dict = RE.OCO_pull_sounding_location(l1b_h5, sounding_id=sounding_id)
    # Get the sounding times
    time_dict = RE.OCO_pull_sounding_time(l1b_h5, sounding_id=sounding_id)
    # Create and get the observers (satellite angles, velocity, position)
    observer_dict = RE.OCO_pull_observer(l1b_h5, sounding_id=sounding_id)

    if !isnothing(cpr_h5)
        @info "Pulling CO2 priors from $(l2cpr_fname)"
        co2_prior_dict = RE.OCO_pull_CO2_prior(cpr_h5, sounding_id=sounding_id)
    end
...
and returns an aerosol property object of type `MieMomAerosolProperty`.
