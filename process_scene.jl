function process_snid(
    ...
    for (spec, swin) in swins
        ...
        reflectance_prior = pi * signal / (solar_strength * cosd(buf.scene.solar_zenith)) * unit_fac
        ...
        for (idx, sve) in RE.StateVectorIterator(state_vector, RE.BRDFPolynomialSVE)
            if sve.swin === swin
                if sve.coefficient_order == 0
                    ...
                    # From the observed reflectance guess, calculate the corresponding
                    # value for the BRDF kernel to create such a reflectance..

                    # First get the surface from the scene that refers the the same
                    # spectral window as this SVE.
                    this_surface = RE.get_surface(buf.scene, sve.swin)

                    sve.prior_value = calculate_kernel_prior(
                            this_surface.kernels[1],
                            buf.scene,
                            reflectance_prior
                            )

                    @info "Estimating BRDF weight for $(swin): $(sve.prior_value)"
...
Estimation of BRDF kernel weight based on the observed reflectance. See
https://doi.org/10.1029/93JD02072 for details on the BRDF itself.

This function will fail for any uplooking observer inside of `scene`.
"""
function calculate_kernel_prior(
    kernel::RE.RPVPolynomialKernel,
    scene::RE.EarthScene,
    reflectance_prior
    )

    # Assign variables to symbols for legibility
    θ1 = scene.solar_zenith # in degrees
    ϕ1 = scene.solar_azimuth # in degrees
    θ2 = scene.observer.viewing_zenith # in degrees
    ϕ2 = scene.observer.viewing_azimuth # in degrees

    ρ0 = kernel.hotspot
    k = kernel.anisotropy
    Θ = kernel.asymmetry

    # Geometric factor G (see Rahman et al. Eq. 6)
    G = sqrt(tand(θ1)^2 + tand(θ2)^2 - 2 * tand(θ1) * tand(θ2) * cosd(ϕ1 - ϕ2))

    # "1 + R(G)", Eq. 5
    one_plus_RG = 1 + (1 - ρ0) / (1 + G)

    # g (Eq. 4)
    g = cosd(θ1) * cosd(θ2) + sind(θ1) * sind(θ2) * cosd(ϕ1 - ϕ2)

    # F(g) (Eq. 3) - NOTE the use of cos() instead of cosd() since the argument is
    # NOT in degrees!!
...
Estimation of BRDF kernel weight based on the observed reflectance. Does nothing for
Lambertian BRDF.
"""
function calculate_kernel_prior(
    kernel::RE.LambertianPolynomialKernel,
    scene::RE.EarthScene,
    reflectance_prior
    )

    return reflectance_prior
